...
export class Car implements ICarItem {
  ...
  public render() {
    ...
    this.removeCar();
    this.selectCarForUpdate(this.id);
  }
  ...
  private addContainersToCar() {
    ...
    car.forEach(elem => elem.append(selectRemoveBtnsContainer, controlButtonsContainer, carImgAndFlagContainer));
  }
  ...
  private renderCarImgAndFlagContainer() {
    ...
    carImgAndFlagContainer.innerHTML = `
      <svg class="car-svg car-svg-${this.id}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="mdi-car-estate" width="24" height="24" viewBox="0 0 24 24" fill = ${this.colorCar}>
  ...
  private removeCar() {
    ...
    const removeButtons = document.querySelectorAll<HTMLButtonElement>('.remove-car');
      ...
      button.addEventListener('click', async () => {
        ...
        cars.forEach((car) => {
          if (Number(car.dataset.id) === Number(button.dataset.id)) {
            dataController.deleteData(Number(button.dataset.id));
  ...
  private async selectCarForUpdate(id: number) {
    const [updateInput, updateColorInput] = ['.input-update-item', '.update-color'].map((item) => document.querySelector<HTMLInputElement>(item));
    const updateButton = document.querySelector<HTMLButtonElement>('.update-item-btn');
    const selectButtons = document.querySelectorAll<HTMLButtonElement>('.select-car');
    ...
    const data = await dataModal.getOrDeleteData('GET', `http://127.0.0.1:3000/garage/${id}`);
    selectButtons.forEach((button) => {
      button.addEventListener('click', () => {
        ...
        updateButton?.addEventListener('click', async () => {
          ...
          const updateData = await dataModal.getOrDeleteData('GET', `http://127.0.0.1:3000/garage/${id}`);
          if (carImg && carName) {
